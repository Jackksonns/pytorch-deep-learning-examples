...
from leavesNet import *  # 导入你自定义的 GoogLeNet 和 resnet_block 等
...
def train_improved(net, train_iter, val_iter, num_epochs, device):
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(net.state_dict(), "best_model_simple.pth")
        else:
            epochs_no_improve += 1
...
if __name__ == '__main__':
    ...
    class leavesNet(nn.Module):  # 自定义融合模型
        def __init__(self):
            super().__init__()
            self.resnet = nn.Sequential(b1, b2, b3, b4, b5)
            self.inception = Inception(512, 128, (256, 256), (384, 256, 256), 256)
            # self.nin = nin_block(896, 176, kernel_size=3, strides=1, padding=1)
            #要先做展平或者池化后才能送入全连接层（也就是说要先全局平均池化+flatten
            self.fc1 = nn.Linear(896,448)
            self.fc2 = nn.Linear(448,224)
            self.dropout = nn.Dropout(0.2)
            self.classifier = nn.Linear(224,176)

        def forward(self, x):
            x = self.resnet(x)  # (batch, C1, H1, W1)
            x = self.dropout(x)
            x = self.inception(x)  # (batch, C2, H2, W2)
            x = self.dropout(x)
            x = F.adaptive_avg_pool2d(x, (1, 1))  # (batch, C2, 1, 1)
            x = torch.flatten(x, 1)  # (batch, C2)
            x = self.fc1(x)
            x = self.fc2(x)
            x = self.dropout(x)
            x = self.classifier(x)
